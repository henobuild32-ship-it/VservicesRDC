...
import {
  ...
  Smartphone, ChevronDown, IdCard, UserCheck, Filter, Zap, Download, Moon, Sun, Palette
} from 'lucide-react'
...
interface ProviderResult { id: string; phone: string; role: AccountType; status?: string;
  profile?: { fullName?: string; companyName?: string; photo?: string; logo?: string; sector?: string;
...
export default function VServiceRDC() {
  ...
  const fetchUserProfile = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setUser({ id: d.id, phone: d.phone, email: d.email, role: d.role, status: d.status, profile: d.profile || undefined })
  ...
  const [regProvince, setRegProvince] = useState(''); const [regCommune, setRegCommune] = useState(''); const [regNationalScope, setRegNationalScope] = useState(false)
  ...
  const searchByType = (type: 'PRESTATAIRE' | 'ENTREPRISE') => { setSearchTargetType(type); setHasSearched(true); setSearchLoading(true) }
  ...
  const viewProviderDetail = async (providerId: string) => {
    ...
    try {
      ...
      if (res.ok) { const d = await res.json(); setProviderDetail({ id: d.id, phone: d.phone, role: d.role, status: d.status, profile: d.profile }); setAvgRating(d.avgRating || 0); setReviewCount(d.reviewCount || 0); setProviderReviews(d.reviews || []); navigate('provider-detail', { providerId }) }
    } catch { /* */ }
    setLoading(false)
  }
  ...
  const [replyMsgId, setReplyMsgId] = useState(''); const [replyText, setReplyText] = useState(''); const [replyLoading, setReplyLoading] = useState(false)
  ...
  const [editProvince, setEditProvince] = useState(''); const [editCommune, setEditCommune] = useState(''); const [editNationalScope, setEditNationalScope] = useState(false)
        ...
        <div className="w-full max-w-lg">
          ...
          <ScrollArea className="max-h-[calc(100vh-200px)]"><Card className="shadow-lg mb-6"><CardContent className="pt-6 space-y-4">
            ...
            {(regType === 'PRESTATAIRE' || regType === 'ENTREPRISE') && <>
              ...
              {!regNationalScope && <div className="grid grid-cols-2 gap-3"><div className="space-y-2"><Label>{t('province')}</Label><Select value={regProvince} onValueChange={v => { setRegProvince(v); setRegCommune('') }}><SelectTrigger className="w-full"><SelectValue /></SelectTrigger><SelectContent>{getAllProvinceNames().map(p => <SelectItem key={p} value={p}>{p}</SelectItem>)}</SelectContent></Select></div><div className="space-y-2"><Label>{t('commune')}</Label><Select value={regCommune} onValueChange={setRegCommune} disabled={!regProvince}><SelectTrigger className="w-full"><SelectValue /></SelectTrigger><SelectContent>{regProvince && getCommunes(regProvince).map(c => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent></Select></div></div>}
            </>}
      ...
      <div className={`min-h-screen ${th.pageBg} pb-20`}>
        ...
        <div className="max-w-lg mx-auto px-4 -mt-10 relative z-10">
          <Card className="shadow-lg"><CardContent className="pt-6">
            ...
            <div className="space-y-2 text-sm">{p?.province && <div className={`flex items-center gap-2 ${th.textSecondary}`}><MapPin className="h-4 w-4" />{p.nationalScope ? 'RDC' : `${p.province}${p.commune ? `, ${p.commune}` : ''}`}</div>}{p?.phone && <div className={`flex items-center gap-2 ${th.textSecondary}`}><Phone className="h-4 w-4" />{p.phone}</div>}{isE && p?.employeeCount && <div className={`flex items-center gap-2 ${th.textSecondary}`}><Users className="h-4 w-4" />{p.employeeCount} employé(s)</div>}{isE && p?.website && <div className={`flex items-center gap-2 ${th.accent}`}><Globe className="h-4 w-4" />{p.website}</div>}</div>
    ...
    <div className={`min-h-screen ${th.pageBg} pb-6`}>
      ...
      <div className="max-w-lg mx-auto px-4 py-4"><ScrollArea className="max-h-[calc(100vh-140px)]"><Card className={th.accentBorder}><CardContent className="pt-6 space-y-4">
        ...
        {user?.role !== 'CLIENT' && <>
          ...
          {!editNationalScope && <div className="grid grid-cols-2 gap-3"><div className="space-y-2"><Label>{t('province')}</Label><Select value={editProvince} onValueChange={v => { setEditProvince(v); setEditCommune('') }}><SelectTrigger className="w-full"><SelectValue /></SelectTrigger><SelectContent>{getAllProvinceNames().map(p => <SelectItem key={p} value={p}>{p}</SelectItem>)}</SelectContent></Select></div><div className="space-y-2"><Label>{t('commune')}</Label><Select value={editCommune} onValueChange={setEditCommune} disabled={!editProvince}><SelectTrigger className="w-full"><SelectValue /></SelectTrigger><SelectContent>{editProvince && getCommunes(editProvince).map(c => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent></Select></div></div>}
  ...
  const renderAdminDashboard = () => (
    <div className={`min-h-screen ${th.pageBg} pb-6`}>
      ...
      <div className={`${th.headerBg} ${th.headerText} px-4 pt-6 pb-6`}>
        <div className="max-w-4xl mx-auto"><div className="flex items-center justify-between mb-4"><div className="flex items-center gap-2"><Shield className="h-6 w-6" /><h1 className="text-xl font-bold">{t('administration')}</h1></div><Button variant="ghost" size="sm" className="text-white hover:bg-white/10" onClick={logout}><LogOut className="h-4 w-4 mr-1" />{t('logout')}</Button></div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">{[{ l: t('totalUsers'), v: adminStats.totalUsers || 0, i: <Users className="h-4 w-4" /> }, { l: t('client') + 's', v: adminStats.clients || 0, i: <User className="h-4 w-4" /> }, { l: t('prestataire') + 's', v: adminStats.prestataires || 0, i: <Wrench className="h-4 w-4" /> }, { l: t('entreprise') + 's', v: adminStats.entreprises || 0, i: <Building2 className="h-4 w-4" /> }, { l: t('pending'), v: adminStats.pending || 0, i: <Clock className="h-4 w-4" /> }, { l: t('suspended'), v: adminStats.suspended || 0, i: <Ban className="h-4 w-4" /> }, { l: t('reviews'), v: adminStats.reviews || 0, i: <Star className="h-4 w-4" /> }, { l: t('messages'), v: adminStats.messages || 0, i: <MessageSquare className="h-4 w-4" /> }].map((s, i) => <div key={i} className="bg-white/10 rounded-lg p-3 backdrop-blur-sm"><div className="flex items-center gap-2 mb-1 text-white/80">{s.i}<span className="text-xs">{s.l}</span></div><p className="text-2xl font-bold">{s.v}</p></div>)}</div>
        </div>
      </div>
      ...
      {/* Suspend dialog */}
      <Dialog open={suspendDialogOpen} onOpenChange={setSuspendDialogOpen}><DialogContent><DialogHeader><DialogTitle>{t('suspend')}</DialogTitle></DialogHeader><div className="space-y-4 py-2"><div className="space-y-2"><Label>Raison</Label><Textarea value={suspendReason} onChange={e => setSuspendReason(e.target.value)} rows={3} /></div></div><DialogFooter><Button variant="outline" onClick={() => setSuspendDialogOpen(false)}>{t('cancel')}</Button><Button className="bg-amber-600 hover:bg-amber-700 text-white" onClick={() => { handleAdminAction(suspendUserId, 'suspend', suspendReason); setSuspendDialogOpen(false); setSuspendReason('') }}>{t('suspend')}</Button></DialogFooter></DialogContent></Dialog>
      {/* User detail dialog */}
      <Dialog open={userDetailOpen} onOpenChange={setUserDetailOpen}><DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto"><DialogHeader><DialogTitle className="flex items-center gap-2"><IdCard className={`h-5 w-5 ${th.accent}`} />{t('userDetails')}</DialogTitle></DialogHeader>{userDetailData && <div className="space-y-4 py-2">
        <div className="flex items-center gap-4"><Avatar className="h-16 w-16 ring-4 ring-emerald-100"><AvatarImage src={userDetailData.profile?.logo || userDetailData.profile?.photo} /><AvatarFallback className={theme === 'red' ? 'bg-red-50 text-red-700' : 'bg-emerald-50 text-emerald-700'}>{getInitials(userDetailData.profile?.companyName || userDetailData.profile?.fullName || 'U')}</AvatarFallback></Avatar><div><h3 className={`font-bold text-lg ${th.textPrimary}`}>{userDetailData.profile?.companyName || userDetailData.profile?.fullName || 'Sans nom'}</h3><div className="flex items-center gap-2 mt-1"><Badge variant="secondary">{userDetailData.role}</Badge>{getStatusBadge(userDetailData.status, theme)}</div></div></div>
        <Separator />
        <div className={`flex items-start gap-3 p-3 rounded-lg ${th.alertBg}`}><Key className="h-4 w-4 text-gray-400 mt-0.5" /><div><p className="text-xs text-gray-500 font-medium">ID</p><p className="text-sm font-mono text-gray-800 break-all">{userDetailData.id}</p></div></div>
        <div className="space-y-2"><h4 className={`text-sm font-semibold ${th.textPrimary}`}>{t('contactInfo')}</h4><div className="space-y-2 pl-1"><div className={`flex items-center gap-2 text-sm ${th.textSecondary}`}><Phone className="h-4 w-4 text-gray-400" />{userDetailData.phone}</div>{userDetailData.email && <div className={`flex items-center gap-2 text-sm ${th.textSecondary}`}><Mail className="h-4 w-4 text-gray-400" />{userDetailData.email}</div>}{userDetailData.profile?.province && <div className={`flex items-center gap-2 text-sm ${th.textSecondary}`}><MapPin className="h-4 w-4 text-gray-400" />{userDetailData.profile.province}{userDetailData.profile.commune ? `, ${userDetailData.profile.commune}` : ''}</div>}</div></div>
        {userDetailData.profile && (<><Separator /><div className="space-y-2"><h4 className={`text-sm font-semibold ${th.textPrimary}`}>Profil</h4><div className="space-y-2 pl-1">{userDetailData.profile.sector && (<div className={`text-sm ${th.textSecondary}`}>Secteur: <strong>{userDetailData.profile.sector}</strong></div>)}{userDetailData.profile.services && userDetailData.profile.services.length > 0 && (<div className="flex flex-wrap gap-1">{userDetailData.profile.services.map((s: string) => (<Badge key={s} variant="secondary" className={theme === 'red' ? 'bg-red-50 text-red-700' : 'bg-emerald-50 text-emerald-700'}>{s}</Badge>))}</div>)}{userDetailData.profile.description && (<p className={`text-sm ${th.textSecondary} bg-gray-50 p-2 rounded`}>{userDetailData.profile.description}</p>)}</div></div></>)}
      </div>}</DialogContent></Dialog>
      {/* Onboarding */}
      <Dialog open={showOnboarding} onOpenChange={(v) => { setShowOnboarding(v); if (!v) { localStorage.setItem('vservicerdc_onboarded', '1'); setOnboardingStep(0) } }}><DialogContent className="max-w-md"><DialogHeader><DialogTitle className={`flex items-center gap-2 ${th.accent}`}>{[t('onboardTitle'), t('onboardStep1'), t('onboardStep2'), t('onboardStep3')][onboardingStep]}</DialogTitle></DialogHeader><div className="py-4 text-center space-y-4">{[<><div className="w-20 h-20 mx-auto rounded-full bg-emerald-50 flex items-center justify-center"><img src="/logo.png" alt="VServiceRDC" className="h-14 w-auto" /></div><p className={th.textSecondary}>{t('onboard0')}</p></>, <><div className={`w-16 h-16 mx-auto rounded-full ${th.accentLight} flex items-center justify-center ${th.accent}`}><Search className="h-8 w-8" /></div><p className={th.textSecondary}>{t('onboard1')}</p></>, <><div className="w-16 h-16 mx-auto rounded-full bg-amber-50 flex items-center justify-center"><Star className="h-8 w-8 text-amber-500" /></div><p className={th.textSecondary}>{t('onboard2')}</p></>, <><div className={`w-16 h-16 mx-auto rounded-full ${th.accentLight} flex items-center justify-center ${th.accent}`}><Bell className="h-8 w-8" /></div><p className={th.textSecondary}>{t('onboard3')}</p></>][onboardingStep]}</div><DialogFooter className="flex-col gap-2"><div className="flex gap-2 w-full">{onboardingStep > 0 && <Button variant="outline" className="flex-1" onClick={() => setOnboardingStep(s => s - 1)}>{t('previous')}</Button>}{onboardingStep < 3 ? <Button className={`flex-1 ${th.primary} ${th.primaryText}`} onClick={() => setOnboardingStep(s => s + 1)}>{t('next')} <ChevronRight className="h-4 w-4 ml-1" /></Button> : <Button className={`flex-1 ${th.primary} ${th.primaryText}`} onClick={() => { setShowOnboarding(false); localStorage.setItem('vservicerdc_onboarded', '1'); setOnboardingStep(0) }}><Check className="h-4 w-4 mr-1" />{t('letsGo')}</Button>}</div><Button variant="ghost" size="sm" className="text-gray-400" onClick={() => { setShowOnboarding(false); localStorage.setItem('vservicerdc_onboarded', '1'); setOnboardingStep(0) }}>{t('skip')}</Button></DialogFooter></DialogContent></Dialog>
    </div>
  )
