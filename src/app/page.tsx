...
export default function VServiceRDC() {
      ...
      {searchResults.map(p => (
        <Card key={p.id} className={'hover:shadow-md transition-shadow ' + th.accentBorder}>
          <CardContent className="p-4"><div className="flex gap-3" onClick={() => viewProviderDetail(p.id)} style={{ cursor: 'pointer' }}>
            ...
            <div className="flex-1 min-w-0"><div className="flex items-start justify-between gap-2"><h3 className={'font-semibold ' + th.textPrimary + ' truncate'}>{p.profile?.companyName || p.profile?.fullName || 'Sans nom'}</h3><div className="flex items-center gap-1">{user && user.role === 'CLIENT' ? <button onClick={(e) => { e.stopPropagation(); toggleFavorite(p.id) }} className="p-0.5"><Heart className={'h-4 w-4 ' + (favorites.includes(p.id) ? 'fill-red-500 text-red-500' : 'text-gray-300 hover:text-red-300')} /></button> : null}{p.status ? getStatusBadge(p.status, theme) : null}{p.certified ? <Badge className="bg-amber-500 text-white text-[10px]"><ShieldCheck className="h-3 w-3 mr-0.5" />Certifie</Badge> : null}</div></div><p className={th.accent}>{p.profile?.sector || ''}</p>{p.profile?.province || p.profile?.nationalScope ? <div className={'flex items-center gap-1 text-xs ' + th.textSecondary + ' mt-1'}><MapPin className="h-3 w-3" />{p.profile?.nationalScope ? 'RDC' : p.profile?.province + (p.profile?.commune ? ', ' + p.profile.commune : '')}</div> : null}{p.profile?.services && p.profile.services.length > 0 ? <div className="flex flex-wrap gap-1 mt-1">{p.profile.services.slice(0, 3).map(s => <Badge key={s} variant="secondary" className={'text-xs ' + (theme === 'red' ? 'bg-red-50 text-red-700' : 'bg-emerald-50 text-emerald-700')}>{s}</Badge>)}</div> : null}</div>
          </div>
  ...
  const renderClientHeader = () => <div className={`${th.headerBg} ${th.headerText} px-4 pt-6 pb-8`}><div className="max-w-lg mx-auto"><div className="flex items-center justify-between mb-4"><div className="flex items-center gap-2"><img src="/logo.png" alt="VServiceRDC" className="h-8 w-auto" /><span className="font-bold text-lg">VServiceRDC</span></div><div className="flex items-center gap-2"><button onClick={() => navigate('notifications')} className="relative p-1.5 rounded-full bg-white/10"><Bell className="h-5 w-5" />{unreadCount > 0 && <span className="absolute -top-0.5 -right-0.5 w-4 h-4 bg-red-500 text-white text-[9px] rounded-full flex items-center justify-center">{unreadCount > 9 ? '9+' : unreadCount}</span>}</button></div></div><h2 className="text-xl font-bold mb-1">{t('hello')}{user?.profile?.fullName ? ', ' + user.profile.fullName : ''}</h2><p className="text-white/80 text-sm">{t('findBestProviders')}</p></div></div>
        ...
        <div className="max-w-lg mx-auto px-4 -mt-10 relative z-10">
          <Card className="shadow-lg"><CardContent className="pt-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mt-4">
              ...
              <div className={`${th.alertBg} rounded-lg p-3 text-center`}><p className={`text-2xl font-bold ${th.accent}`}>{realisations.length}</p><p className={`text-xs ${th.textSecondary}`}>Realisations</p></div>
            </div>
            ...
            {/* Shareable link */}
            <Button variant="outline" className={`w-full mt-2 ${th.accentBorder} ${th.accent}`} onClick={async () => { 
              try {
                let slug = ''
                if (isE) {
                  const r = await fetch('/api/public/me', { headers: authHeaders() })
                  if (r.ok) { const d = await r.json(); slug = d.slug }
                } else {
                  const r = await fetch('/api/public/me', { headers: authHeaders() })
                  if (r.ok) { const d = await r.json(); slug = d.slug }
                }
                const url = `${window.location.origin}/${isE ? 'entreprise' : 'prestataire'}/${slug || user?.id}`
                navigator.clipboard.writeText(url)
                toast({ title: 'Lien copié !', description: url })
              } catch { 
                const url = `${window.location.origin}/${isE ? 'entreprise' : 'prestataire'}/${user?.id}`
                navigator.clipboard.writeText(url)
                toast({ title: 'Lien copié !', description: url })
              }
            }}><ExternalLink className="h-4 w-4 mr-1" />Generer mon lien</Button>
          ...
          {/* Employee management for companies */}
          {isE && <div className="mt-6"><div className="flex items-center justify-between mb-3"><h2 className={`text-lg font-bold ${th.textPrimary}`}>Employes</h2><Button size="sm" className={`${th.primary} ${th.primaryText} text-xs h-7`} onClick={() => { setEmpOpen(true); setEmpName(''); setEmpFunction(''); setEmpPhone(''); setEmpEmail('') }}><Plus className="h-3 w-3 mr-1" />Ajouter</Button></div>
            <div className="space-y-2 max-h-80 overflow-y-auto">{employees.length === 0 ? <p className={`text-sm text-gray-400 text-center py-4`}>Aucun employe</p> : employees.map(e => <Card key={e.id}><CardContent className="p-3"><div className="flex items-center justify-between gap-2"><div className="flex items-center gap-2 min-w-0"><Avatar className="h-8 w-8 flex-shrink-0"><AvatarFallback className={theme === 'red' ? 'bg-red-50 text-red-700' : 'bg-emerald-50 text-emerald-700'}>{getInitials(e.fullName)}</AvatarFallback></Avatar><div className="min-w-0"><p className={`font-medium text-xs sm:text-sm truncate ${th.textPrimary}`}>{e.fullName} {e.isManager && <Badge className="bg-amber-500 text-white text-[8px] ml-1">Responsable</Badge>}</p><p className={`text-xs ${th.textSecondary}`}>{e.function}</p></div></div><Button size="sm" variant="ghost" className="h-7 w-7 p-0 text-red-400" onClick={async () => { if (!confirm('Supprimer ' + e.fullName + ' ?')) return; try { await fetch('/api/employees?id=' + e.id, { method: 'DELETE', headers: authHeaders() }); fetchEmployees() } catch { /* */ } }}><Trash2 className="h-3 w-3" /></Button></div></CardContent></Card>)}</div>
          </div>}
  ...
  const renderAdminDashboard = () => (
    <div className={`min-h-screen ${th.pageBg} pb-6`}>
      ...
      <div className="max-w-4xl mx-auto px-4 mt-6">
        <Tabs value={adminTab} onValueChange={setAdminTab}>
          <div className="overflow-x-auto -mx-4 px-4 mb-4 scrollbar-hide">
            <TabsList className="inline-flex w-auto min-w-max gap-1">
              ...
              <TabsTrigger value="publications" className="text-xs sm:text-sm whitespace-nowrap px-2 sm:px-3">Publications</TabsTrigger>
        ...
        {/* Individual message */}
        <Card className="mt-4"><CardHeader className="p-3 sm:p-6"><CardTitle className="text-sm sm:text-base">Message individuel</CardTitle></CardHeader><CardContent className="space-y-3 p-3 sm:p-6 pt-0 sm:pt-0"><div className="space-y-1 sm:space-y-2"><Label className="text-xs sm:text-sm">Envoyer a</Label><Select value={annTargetId || ''} onValueChange={setAnnTargetId}><SelectTrigger className="text-xs sm:text-sm h-8 sm:h-9"><SelectValue placeholder="Choisir un utilisateur" /></SelectTrigger><SelectContent><SelectItem value="">-- Selectionner --</SelectItem>{adminUsers.filter(u => u.role !== 'ADMIN').map(u => <SelectItem key={u.id} value={u.id}>{u.profile?.fullName || u.profile?.companyName || u.phone} ({u.role})</SelectItem>)}</SelectContent></Select></div><div className="space-y-1 sm:space-y-2"><Label className="text-xs sm:text-sm">Message</Label><Textarea value={indivMsg} onChange={e => setIndivMsg(e.target.value)} rows={3} className="text-xs sm:text-sm" placeholder="Votre message..." /></div><Button className={th.primary + ' ' + th.primaryText + ' text-xs sm:text-sm h-8 sm:h-9'} onClick={async () => { if (!annTargetId || !indivMsg) return; try { const r = await fetch('/api/admin/announcements', { method: 'POST', headers: authHeaders(), body: JSON.stringify({ title: 'Message de l\'administration', message: indivMsg + '\n\n---\n**Équipe VServicesRDC**', targetType: 'specific', targetId: annTargetId }) }); if (r.ok) { toast({ title: 'Message envoye' }); setIndivMsg(''); fetchAdminData() } } catch { /* */ } }} disabled={!annTargetId || !indivMsg}><Send className="h-3 w-3 sm:h-4 sm:w-4 mr-1" />Envoyer le message</Button></CardContent></Card></TabsContent>
