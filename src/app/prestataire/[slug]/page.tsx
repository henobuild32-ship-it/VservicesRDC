...
import { MapPin, Phone, Star, ShieldCheck, MessageSquare, ChevronLeft, Globe, Image } from 'lucide-react'
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-emerald-50 to-white pb-8">
      <div className="max-w-lg mx-auto px-4 pt-4">
        <a href="/" className="inline-flex items-center gap-1 text-sm text-emerald-600 hover:text-emerald-700 mb-4"><ChevronLeft className="h-4 w-4" />Retour</a>
        <Card className="shadow-lg overflow-hidden">
          <CardContent className="pt-6 pb-6">
            <div className="flex items-start gap-4 mb-4">
              <Avatar className="h-20 w-20 flex-shrink-0 ring-4 ring-white shadow"><AvatarImage src={p?.photo} /><AvatarFallback className="bg-emerald-50 text-emerald-700 text-lg">{getInitials(p?.fullName || 'P')}</AvatarFallback></Avatar>
              <div className="flex-1">
                <h1 className="text-xl font-bold text-gray-900">{p?.fullName}</h1>
                <p className="text-emerald-600 font-medium text-sm">{p?.sector}</p>
                ...
                {p?.services && p.services.length > 0 && <div className="flex flex-wrap gap-1 mt-2">{p.services.map((s: string) => <Badge key={s} variant="secondary" className="text-xs bg-emerald-50 text-emerald-700">{s}</Badge>)}</div>}
              </div>
            </div>
            {p?.description && <p className="text-gray-600 text-sm mb-4 whitespace-pre-wrap">{p.description}</p>}
            <Separator className="my-4" />
            <div className="space-y-2 text-sm">
              <div className="flex items-center gap-2 text-gray-600"><Phone className="h-4 w-4 text-emerald-500" /><a href={'tel:' + data.phone} className="hover:underline">{formatPhone(data.phone)}</a></div>
              <div className="flex items-center gap-2 text-gray-600"><MapPin className="h-4 w-4 text-emerald-500" />{p?.nationalScope ? 'RDC' : [p?.province, p?.commune].filter(Boolean).join(', ')}</div>
            </div>
            <Separator className="my-4" />
            ...
            <a href={'tel:' + data.phone} className="w-full inline-flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg px-4 py-2.5 text-sm font-medium mb-2"><Phone className="h-4 w-4" />Appeler maintenant</a>
            <a href={'/'} className="w-full inline-flex items-center justify-center gap-2 border border-emerald-200 text-emerald-600 hover:bg-emerald-50 rounded-lg px-4 py-2.5 text-sm font-medium"><MessageSquare className="h-4 w-4" />Créer un compte pour contacter</a>
